...
while true; do
    ...
    case $choice in
        1)
            ...
            echo
            echo "wget -O en-new-ssh-bash.sh https://github.com/explorenym/en-new-ssh-bash.sh && chmod +x en-new-ssh-bash.sh && . ~/en-new-ssh-bash.sh $new_user@$public_ip"
            return
            ;;
        2)
            echo "Paste this code in your local Powershell:"
            echo
            echo "Invoke-WebRequest \"https://github.com/explorenym/en-new-ssh-shell.ps1\" -OutFile \"en-new-ssh-shell.ps1\"
