...
case "$(groups)" in
  ...
  *root*)
    ...
    wget -O en-new-user.sh https://github.com/explorenym/en-new-user.sh && chmod +x en-new-user.sh &&  . ~/en-new-user.sh
    return
    fi
    ;;
  *)
...
mix_node=$(find "$mixnodes_dir" -mindepth 1 -maxdepth 1 -type d -printf "%f\n" 2>/dev/null | head -n1)
  ...
  while true; do
    ...
    case $choice in
        1)
            wget -O en-mix-install.sh https://github.com/explorenym/en-mix-install.sh && chmod +x en-mix-install.sh &&  . ~/en-mix-install.sh
            return
            ;;
        2)
  ...
  while true; do
    ...
    case $choice in
        1)
            wget -O en-mix-update.sh https://github.com/explorenym/en-mix-update.sh && chmod +x en-mix-update.sh &&  . ~/en-mix-update.sh
            return
            ;;
        2)
            wget -O en-mix-back.sh https://github.com/explorenym/en-mix-back.sh && chmod +x en-mix-back.sh &&  . ~/en-mix-back.sh
            return
            ;;
        3)
            wget -O en-node-exporter.sh https://github.com/explorenym/en-node-exporter.sh && chmod +x en-node-exporter.sh &&  . ~/en-node-exporter.sh
